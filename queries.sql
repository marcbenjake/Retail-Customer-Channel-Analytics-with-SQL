-- ============================================================
-- From Clicks to Customers: A SQL Analysis of E-Commerce
-- Marketing Performance
-- ============================================================
-- Industry:  E-Commerce / Retail
-- Tool:      SQL Server
-- Dataset:   5,000 orders | 500 customers (synthetic)
-- ============================================================


-- ============================================================
-- BEGINNER: Q1 - Q5
-- Core SQL fluency - aggregations, grouping, filtering,
-- and CASE WHEN logic
-- ============================================================
...
-- ============================================
-- Goal: Count orders and sum revenue by marketing channel to identify
-- the most effective channels for driving both volume and revenue.
...
-- Insight: Organic Search dominates both order count (1,110) and total revenue
-- ($398,592), making it the single most valuable channel by far.
-- Social Media ranks second in orders (1,028) but its revenue ($373,021)
-- is closer to Organic Search, suggesting similar average order values.
-- Paid Search drives fewer orders (937) but generates strong revenue ($362,359),
-- hinting at higher intent buyers. Referral sits at the bottom with only
-- 155 orders ($37,176), indicating an underdeveloped channel worth investigating.
...
-- ============================================
-- Goal: Compare average net revenue, discount amount, and order quantity
-- between orders that used a promo code vs those that didn't.
...
-- Insight: Orders with promo codes generate slightly higher average net revenue
-- ($372.46) compared to non-promo orders ($356.33), suggesting promo codes
-- attract higher value purchases rather than just discounting existing ones.
-- Average discount amounts are nearly identical ($28.80 vs $28.40), indicating
-- promo codes don't significantly deepen discounting. Order quantity is the same
-- across both groups at 1, suggesting promos don't drive bulk purchasing behavior.
-- Overall, promo codes appear to attract higher spending customers rather than
-- simply cannibalizing margin.


-- ============================================
-- Q5. What is the split between device types and does it affect order size?
-- ============================================
-- Goal: Group by device type and compare order count, average order size,
-- average net revenue and pages viewed to identify behavioral differences.
...
-- Insight: Mobile dominates order volume with 2,680 orders (53% of all orders),
-- confirming a mobile-first customer base. However Mobile's average net revenue
-- ($374.69) is the highest across all devices, challenging the assumption that
-- mobile users spend less. Desktop accounts for 1,942 orders with a slightly
-- lower AOV of $358.12. Tablet has the lowest order volume (378) and the lowest
-- AOV ($322.32). Pages viewed is consistent across all three devices at 11-12,
-- suggesting browsing behavior is not significantly influenced by device type.
-- The high mobile order volume and AOV suggests the storefront is well optimised
-- for mobile commerce.


-- ============================================================
-- INTERMEDIATE: Q6 - Q10
-- Working across tables analytically - JOINs, subqueries,
-- and segmentation
-- ============================================================
...
cte2 AS (
    SELECT
        customer_id,
        acquisition_channel,
        ABS(DATEDIFF(day, signup_date, first_order_date)) AS days_to_first_order
    FROM cte1
)
SELECT
    acquisition_channel,
...
-- ============================================================
-- ADVANCED: Q11 - Q16
-- Analytical thinking beyond querying - window functions,
-- CTEs, and business metric calculations
-- ============================================================
